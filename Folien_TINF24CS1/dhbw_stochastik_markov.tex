...
\[
\PP(X_{n+1} = j \mid X_n = i, X_{n-1}, \ldots, X_0) = p_{ij}
\]
...
\[
\overline{P}_n \cdot P = \overline{P}_n + \tfrac{1}{n}(P^n - I) \;\xrightarrow{n\to\infty}\; Q
\]
...
\item Vorschlagsverteilung $q(y \mid x)$ (leicht zu sampeln)
\end{itemize}
\end{block}
\begin{block}{Algorithmus}
Gegeben $X_n = x$:
\begin{enumerate}
\item Schlage $y \sim q(\cdot \mid x)$ vor.
\item Berechne Akzeptanzwahrscheinlichkeit:
\[
\alpha(x, y) = \min\!\left(1, \;\frac{\pi(y)\, q(x \mid y)}{\pi(x)\, q(y \mid x)}\right)
\]
\item Mit W'keit $\alpha$: setze $X_{n+1} = y$. Sonst: $X_{n+1} = x$.
\end{enumerate}
\end{block}
 \end{frame}
...
\framesubtitle{Metropolis-Hastings -- Korrektheit}
    \begin{block}{Satz: $\pi$ erfüllt Detailed Balance}
Die Übergangsmatrix $P$ des Metropolis-Hastings-Algorithmus erfüllt:
\[
\pi(x)\, P(x,y) = \pi(y)\, P(y,x) \quad \text{für alle } x \neq y
\]
\end{block}
\begin{block}{Beweis}
Für $x \neq y$: $P(x,y) = q(y\mid x) \cdot \alpha(x,y)$. O.B.d.A.\ $\pi(x)q(y|x) \geq \pi(y)q(x|y)$:
\begin{align*}
\pi(x) P(x,y) &= \pi(x)\, q(y|x) \cdot \frac{\pi(y)\,q(x|y)}{\pi(x)\,q(y|x)} = \pi(y)\,q(x|y) \\
\pi(y) P(y,x) &= \pi(y)\, q(x|y) \cdot \min\!\left(1, \frac{\pi(x)q(y|x)}{\pi(y)q(x|y)}\right) = \pi(y)\,q(x|y)
\end{align*}
$\Rightarrow$ Detailed Balance $\Rightarrow$ $\pi$ ist invariant! \hfill $\square$
\end{block}
 \end{frame}
...
\framesubtitle{Monte-Carlo-Integration}
    \begin{block}{Problem}
Berechne $\E_\pi[g] = \sum_{x \in S} g(x)\, \pi(x)$ \quad (oder $\int g(x)\,\pi(x)\,dx$)\\[0.1cm]
Oft ist $|S|$ riesig oder $\pi$ nur bis auf Normierung bekannt.
\end{block}
\begin{block}{Lösung: Ergodentheorem}
Ist $(X_n)$ eine irreduzible, aperiodische Markov-Kette mit invarianter Verteilung $\pi$, so gilt:
\[
\frac{1}{N} \sum_{n=1}^{N} g(X_n) \;\xrightarrow{N \to \infty}\; \E_\pi[g] \quad \text{(fast sicher)}
\]
\end{block}
\begin{block}{Praxis}
Simuliere Kette, berechne Durchschnitt $\Rightarrow$ approximiert Erwartungswert!
\end{block}
 \end{frame}

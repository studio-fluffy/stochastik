...
Für $n \geq 2$ sei  $\biggl ( \mathbb{R}^n, \mathcal{B}(\mathbb{R}^n ), P^n_\rho :=  \prod (P_\rho)_i \biggr)$ das Produktmodell,  $X_i(x_1, \cdot , x_n) = x_i$ die Projektion auf die $i$-te Koordinate  und $m(\rho) = \mathbb{E}(X) $ so wie $v(\rho) =\mathbb{V}(X)$. Dann sind das Stichprobenmittel $M= \frac{1}{n} \sum_{i=1}^n X_i$ und die Stichprobenvarianz
 $V= \frac{1}{n-1} \sum_{i=1}^n (X_i - M)^2$ erwartungstreue Schätzer für $m:= \mathbb{E}(id)$ bzw. $v: = \mathbb{V}(id)$.
\end{block}
